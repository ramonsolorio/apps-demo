IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.SP_GET_PROMOTION_DATA_XML'))
	DROP PROCEDURE [dbo].[SP_GET_PROMOTION_DATA_XML]
GO

CREATE PROCEDURE [dbo].[SP_GET_PROMOTION_DATA_XML]
...
BEGIN
	...
	--WITH XMLNAMESPACES ('rawxml' AS ns1)
	SELECT 
		(SELECT TOP 1
			'MX' AS 'organizationCountry',
			'FEMSA Comercio' AS 'organizationName',
			'OXXO' AS 'businessName',
			[ORACLE_CR_SUPERIOR] AS 'departmentId',
			[ORACLE_CR] AS 'storeId',
			(SELECT count(*) FROM #@TableName) AS 'numberOfRecords',
			'000000000000000000000000' AS 'searchId',
			GETDATE() AS 'searchDate',
			[ORACLE_CR],
			[ORACLE_CR_DESC],
			[ORACLE_CR_SUPERIOR],
			[ORACLE_CR_TYPE],
			...
			[LEGACY_EF],
			[LEGACY_CR]
		 FROM [dbo].[CO_ERDD_MCRS] x
		 WHERE RETEK_CR = @Location
		 FOR XML PATH(''), TYPE, ELEMENTS),
		 (SELECT
				ROW_NUMBER() OVER(ORDER BY t.[LOAD_BATCH_ID] DESC, t.[STORE], t.[LOAD_TIMESTAMP], t.[PROMOTION] ASC) AS 'lineNumber',
				t.[PROMOTION] AS 'folio',
				'' AS 'itemCategory',
				'' AS 'type',
				FORMAT(t.[START_DATE], 'yyyyMMdd') AS 'startDate',
				FORMAT(t.[END_DATE], 'yyyyMMdd') AS 'endDate',
				t.[ITEM] AS 'itemSku',
				t.[MANUAL_PRICE_ENTRY] AS 'category',
				'' AS 'providerType',
				'' AS 'requiredQuantity',
				'' AS 'discount',
				'' AS 'discountType',
				'' AS 'optative',
				t.[PROM_NAME] AS 'description',
				t.[DEPT] AS 'itemSubcategory',
				t.[CLASS] AS 'itemSegment',
				t.[SUBCLASS] AS 'itemSubsegment',
				'' AS 'durationCode',
				t.[REF_PREFIX] AS 'grouper',
				'' AS 'percent',
				'' AS 'optativeOne',
				'' AS 'sent',
				'' AS 'action',
				t.[PROM_BIN_CODE] AS 'binCode',
				t.[MSG_POS] AS 'messagePsg',
				t.[TRAN_TYPE] AS 'tranType',
				t.[PROM_TRAN_TYPE] AS 'promTranType',
				t.[THRESHOLD_NO] AS 'thresHold_no',
				t.[THRESHOLD_AMT] AS 'thresHoldAmt',
				t.[NEW_PRICE] AS 'newPrice',
				t.[DISCOUNT_AMT] AS 'discountAmt',
				t.[NEW_SELLING_UOM] AS 'newSellingUom',
				t.[DEPT] AS 'dept',
				t.[CLASS] AS 'class',
				t.[SUBCLASS] AS 'subClass',
				t.[REF_PREFIX] AS 'refPrefix',
				t.[PREFIX] AS 'prefix',
				t.[MANUAL_PRICE_ENTRY] AS 'manualPriceEntry',
				t.[MSG_POS] AS 'msgPos',
				t.[START_TIME] AS 'startTime',
				t.[END_TIME] AS 'endTime',
				t.[ITEM_SHORT_DESC] AS 'shortDesc',
				t.[PROM_BIN_CODE] AS 'promBinCode'
		FROM #@TableName t
		ORDER BY t.[LOAD_BATCH_ID] DESC, t.[STORE], t.[LOAD_TIMESTAMP], t.[PROMOTION] ASC
		FOR XML PATH('PromotionData'), ROOT('Promotions'), TYPE, ELEMENTS XSINIL)
	FOR XML PATH('PromotionBatch'), TYPE;
END